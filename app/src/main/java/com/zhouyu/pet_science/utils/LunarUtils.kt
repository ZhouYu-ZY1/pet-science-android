...
class LunarUtils(cal: Calendar) {
    ...
    val lunarDay: String
        get() = getChinaDayString(day)
    val lunarMonth: String
        get() = chineseNumber[month - 1]

    override fun toString(): String {
        return (lunarYear.toString() + "年" + (if (leap) "闰" else "") + chineseNumber[month - 1] + "月"
    ...
    companion object {
        ...
        private fun leapDays(y: Int): Int {
            return if (leapMonth(y) != 0) {
                if (lunarInfo[y - 1900] and 0x10000L != 0L) 30 else 29
            } else 0
        }

        // ====== 传回农历 y年闰哪个月 1-12 , 没闰传回 0
        private fun leapMonth(y: Int): Int {
            return (lunarInfo[y - 1900] and 0xfL).toInt()
        }
